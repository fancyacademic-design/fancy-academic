...
export default function SendNotificationPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        snapshot.forEach(doc => {
          const data = doc.data();
          if (data.role === 'student' && (data.status === 'active' || data.status === 'مفعل')) {
            studentsList.push({
              id: doc.id,
              name: data.name,
              grade: data.grade,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ بناء بيانات الإشعار بالهيكل الصحيح
      const notificationData: any = {
        title: title.trim(),
        body: body.trim(),
        type: 'admin_notification', // ✅ نوع الإشعار
        target: {
          type: targetType,
        },
        readBy: [], // ✅ لتتبع القراءة
      ...
      if (targetType === 'grade' && selectedGrade) {
        notificationData.target.grade = selectedGrade;
      } else if (targetType === 'student' && selectedStudent) {
        const student = students.find(s => s.id === selectedStudent);
        notificationData.target.studentId = selectedStudent;
        notificationData.target.studentName = student?.name || 'طالب';
        // ✅ إضافة studentId مباشرة للبحث السريع
        notificationData.studentId = selectedStudent;
      }

      // ✅ إضافة نوع الإشعار للتمييز
      notificationData.notificationType = type; // info, success, warning, error

      // ✅ إضافة اسم المرسل
      notificationData.sender = 'أدمن المنصة';
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.preview}>
          ...
          <div style={{
            ...
            <p style={styles.previewBody}>{body || 'محتوى الإشعار سيظهر هنا...'}</p>
          </div>
        </div>
